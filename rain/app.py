...
st.write("أدخل بيانات الطقس اليوم للتنبؤ باحتمالية هطول المطر غدًا (بيانات أستراليا).")
...
with col1:
    location = st.selectbox("المدينة (Location)", locations, index=locations.index("Sydney") if "Sydney" in locations else 0)
    ...
    rainfall = st.number_input("كمية المطر اليوم (Rainfall mm)", min_value=0.0, max_value=300.0, value=0.0, step=0.5)
...
if st.button("🔮 تنبأ بالمطر غدًا", use_container_width=True, type="primary"):
    ...
    if prediction == 1:
        st.success(f"🌧️ الأرجح إنها **هتمطر بكرة**  |  احتمالية المطر: {probability:.1%}")
    else:
        st.info(f"☀️ الأرجح إنها **مش هتمطر بكرة**  |  احتمالية المطر: {probability:.1%}")
...
st.divider()
st.caption("مبني باستخدام Logistic Regression / KNN مع Scikit-learn Pipeline. المشروع النهائي - Rain Prediction (Australia).")
