...
@st.cache_resource
def load_model():
    # الحصول على المسار المطلق للمجلد الذي يحتوي على هذا الملف (app.py)
...
with col2:
    sibsp = st.number_input(
        "عدد الإخوة/الزوج على متن السفينة (SibSp)",
        min_value=0,
        max_value=8,
        value=0,
    )
    parch = st.number_input(
        "عدد الأبناء/الآباء على متن السفينة (Parch)",
        min_value=0,
        max_value=6,
        value=0,
    )
...
if st.button("🔮 تنبأ بالنجاة", use_container_width=True, type="primary"):
    ...
    if prediction == 1:
        st.success(
            f"✅ الراكب على الأرجح **نجا**  |  احتمالية النجاة: {probability:.1%}"
        )
    else:
        st.error(
            f"❌ الراكب على الأرجح **لم ينجُ**  |  احتمالية النجاة: {probability:.1%}"
        )
...
st.caption(
    ...
    " النهائي - Titanic Classification."
)
